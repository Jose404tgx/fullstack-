...
import { getDetalleVentas, createDetalleVenta, deleteDetalleVenta, getVentas, getProductos } from '../services/api';
        ...
        <tbody>
          {data.map(item => (
            <tr key={item.id_detventa}>
              ...
              <td>{item.id_venta}</td>
               <td>{item.descripcion || item.id_producto}</td>
               <td>{item.precio || '-'}</td>
              <td>{item.cantidad}</td>
              <td>
